/**
 * Danh mục dùng chung (Droplist): Nhân sự, Công ty, Đối tác, Dự án.
...
const MASTERDATA_FALLBACK = { employees: [], companies: [], partners: [], projects: [] };
const OTHER_VALUE = "__other__";

const MD_CATEGORIES = [
  { key: "employees", tkey: "md.cat.employees" },
  { key: "companies", tkey: "md.cat.companies" },
  { key: "partners", tkey: "md.cat.partners" },
...
const MasterData = {
  ...
  async saveAll(api) {
    const res = await api.updateJsonFile(
      ...
      "AdminOps: update master data (droplists)"
    );
  ...
  async quickAdd(api, category, value) {
    ...
    try {
      await api.updateJsonFile(
        ...
        `AdminOps: add "${value}" to ${category}`
      );
    } catch (e) {
